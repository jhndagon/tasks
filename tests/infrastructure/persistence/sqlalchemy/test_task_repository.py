...
def test_repository_list_filters_by_title_starts_with(tmp_path) -> None:
    async def scenario() -> None:
        engine, session_factory = await _build_session_factory(
            f"sqlite+aiosqlite:///{tmp_path / 'repo_list_title_starts_with.db'}"
        )
        try:
            async with session_factory() as session:
                repository = SQLAlchemyTaskRepository(session)
                first = await repository.create(title=TaskTitle("Comprar fruta"))
                await repository.create(title=TaskTitle("Ir a comprar"))
                third = await repository.create(title=TaskTitle("Comprometer agenda"))

                matches = await repository.list(title_starts_with="comp")
